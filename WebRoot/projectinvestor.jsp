...
<%
	...
	String requesturl = "projectinvestor.jsp";
   ...
   totalRecords = OrderMgr.getInstance().getVOsByProjectId(ordervos, pageNo, PAGE_SIZE, pk_project);
...
<div class="bg-white clearfix">
	...
	<div class="particulars clearfix">
       ...
       <tbody><tr><th>投资者</th><th>金额</th><th>类型</th><th>下单时间</th></tr>
       <%
       for(Iterator<OrderVO>it = ordervos.iterator();it.hasNext();){
       OrderVO ordervo = it.next();
        %>
              <tr><td class="fw"><%=UserMgr.getInstance().getUserVOById(ordervo.getPk_user()).getUsername() %></td><td class="fc">¥<%=ordervo.getTotalsum() %></td><td><%=ordervo.getOrdertype() %></td><td><%=new SimpleDateFormat("yyyy-MM-dd").format(ordervo.getPublishtime()) %></td></tr>
             
       <%} %>   
         </tbody></table>
         <div class="pager">

						<a class="prev" href="projectinvestor.jsp?pageNo=1&pk_project=<%=pk_project%>">首页</a> <a
							class="prev"
							href="projectinvestor.jsp?pageNo=<%=pageNo == 1 ? 1 : pageNo - 1%>&pk_project=<%=pk_project%>">&lt;</a>
						&nbsp;
						<%
							for (int i = 1; i <= totalPages; i++) {
						%>
						<a class="<%=(i == pageNo) ? "selected" : "common-sprite"%>"
							href="projectinvestor.jsp?pageNo=<%=i%>&pk_project=<%=pk_project%>"><%=i%></a>&nbsp;
						<%
							}
						%>
						<a class="next"
							href="projectinvestor.jsp?pageNo=<%=pageNo == totalPages ? totalPages : pageNo + 1%>&pk_project=<%=pk_project%>">&gt;</a>
						<a class="common-sprite"
							href="projectinvestor.jsp?pageNo=<%=totalPages%>&pk_project=<%=pk_project%>">尾页</a>
					</div>
				</div>
			</div>
				<div class="m-portpe">
...
</style><div class="support-project-list">
				<h4>同类推荐</h4>
				<ul>
