...
<%
     ...
     String pk_userstr = request.getParameter("pk_user");
     int pk_user = 0;
     UserVO uservo = new UserVO();
     
     if(pk_userstr != null && !pk_userstr.trim().equals(""))
     {
     pk_user = Integer.parseInt(pk_userstr);
	...
	try{
	 if (UserMgr.getInstance().updateUser(uservo) == 0)
	 {
	 response.getWriter()
	 .print("<script>alert('更新成功!');window.location.href='admin_manage_user.jsp'</script>");

	 }
	 }catch(UserNameExistException e){
	 e.printStackTrace();
	  response.getWriter()
	 .print("<script>alert('用户名重复!');window.location.href='admin_update_user.jsp'</script>");
	 }
	 }
	...
	<div class="backstage-wrap clearfix">
		...
		<div class="backstage-right" style="min-height:650px;" id="backcolor">
			...
			<div class="personal-set-right">
				<form class="setting-form" name="addForm" action="admin_update_user.jsp">
					...
					<div class="form-item clearfix">
						...
						<div class="row clearfix">
							<label>用户名：</label> <input name="name" type="text"
								class="input-wid"  value="<%=uservo.getUsername()%>">
						</div>
						
						<div class="row clearfix">
							<label>性别：</label> <span class="span-style"> <label
								class="label-style"> <input type="radio" name="sex" checked="<%=uservo.getSex().equals("男")?"checked":""%>"
									value="男"> 男</label> <label class="label-style"> <input
									type="radio" checked="<%=uservo.getSex().equals("女")?"checked":""%>" name="sex" value="女"> 女</label>
							</span>
						</div>
						<div class="row clearfix">
							<label>手机号：</label> <input name="tel" type="text"
								class="input-wid" value="<%=uservo.getTel()%>" >
						</div>
						<div class="row clearfix">
							<label>邮箱：</label> <input type="text" 	name="email" value="<%=uservo.getEmail()%>" >
						</div>

						<div class="row clearfix">
							<label>个人说明：</label>
							<textarea name="resume" ><%=uservo.getResume()%></textarea>
						</div>
