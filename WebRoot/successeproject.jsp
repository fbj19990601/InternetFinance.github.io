...
<%
	...
	ProjectBrowseSearchVO searchvo = (ProjectBrowseSearchVO) session
			.getAttribute("successsearchvo");
   List<ProjectVO> projectvos = new ArrayList<ProjectVO>();
   
	if(searchvo != null){
	
	totalRecords = ProjectMgr.getInstance().getSuccessProjects(projectvos, pageNo, PAGE_SIZE, searchvo);
	
	}else{
	  totalRecords = ProjectMgr.getInstance().getSuccessProjects(projectvos, pageNo, PAGE_SIZE);
	}
	...
	<div class="wrap m-box ">
			...
			<ul>
				<li class="li-font">分类排序：</li>
					<%
						...
						%>
				<li><a href="successesearch.jsp?catagoryid=<%=projectcatagoryvo.getCatagoryid()%>"><%=projectcatagoryvo.getCatagoryname()%></a></li>
		...
		<!-- <div class="type-list clearfix">
			<ul>
				...
				<li><a href="successseearch.jsp?other=默认排序">默认排序</a>
				</li>
				<li><a href="successesearch.jsp?other=最新上线">最新上线</a>
				</li>
				<li><a href="successesearch.jsp?other=最高目标金额">最高目标金额</a>
				</li>
				<li><a href="successesearch.jsp?other=最多关注人数">最多关注人数</a>
				</li>
				<li><a href="successesearch.jsp?other=最多完成金额">最多完成金额</a>
				</li>
		...
		<div class="type-list clearfix">
			<ul>
				...
				%>
				<li><a
					href="successesearch.jsp?catagoryid=<%=searchvo.getCatagoryid()%>&action=delete"><%=ProjectCatagoryMgr.getInstance()
				...
				%>
				<li><a href="successesearch.jsp?other=<%=searchvo.getOther()%>&action=delete"><%=searchvo.getOther()%>x</a></li>
				<%
					}
					if(searchvo.getCatagoryid() == 0&& searchvo.getOther().equals("")) {
				%>
