...
<%
	...
	if (searchvo == null) {
		...
		session.setAttribute("successsearchvo", searchvo1);
	} else {
		if (action != null && action.trim().equals("delete")) {
			...
			session.setAttribute("successsearchvo", searchvo);
		} else {
			if (catagoryidstr != null
					&& !catagoryidstr.trim().equals("")) {
				catagoryid = Integer.parseInt(catagoryidstr);
	...
	response.getWriter()
			.print("<script>window.location.href='successeproject.jsp'</script>");
%>