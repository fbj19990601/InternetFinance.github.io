...
<%@page import="com.lechou.project.comment.ProjectCommentMgr"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page
	import='com.lechou.project.*,com.lechou.user.*,com.lechou.project.approvelist.*,com.lechou.project.catagory.*,com.lechou.project.latestnews.*,com.lechou.order.*,java.util.*'%>
...
<%
	...
	String requesturl = "projectdetail.jsp";
    ...
    hotestprojectvos = ProjectMgr.getInstance().getHostestProject(hotestprojectbycatacount);
     ...
     List<ProjectVO> hotapproveprojectvos = new ArrayList<ProjectVO>();
    int hotapproveprojectcount = 4;
    hotapproveprojectvos = ProjectMgr.getInstance().getHotApproveProject(hotapproveprojectcount);
    
     List<ProjectVO> hotapprovebycatagroyprojectvos = new ArrayList<ProjectVO>();
    int hotapprovebycatagroycount = 4;
    hotapprovebycatagroyprojectvos = ProjectMgr.getInstance().getHotApproveProjectByCatagoryid(hotapprovebycatagroycount, projectvo.getCatagoryid());
	...
	<div class="particulars clearfix">
		...
		<div class="particulars-left">
			<div id="Js-detail" class="m-matter white">
				...
				<div class="content">
					<!--视频放置区-->
					<%if(projectvo.getMovieurl() != null && !projectvo.getMovieurl().equals("")) {%>
					<div id="movie">
						<script src="js/movie/AC_ActiveX.js" type="text/javascript"></script>
						<script src="js/movie/AC_RunActiveContent.js"
						...
						<script type="text/javascript">
							AC_AX_RunContent(
									...
									'true',
									'flashvars',
									'vcastr_file=uploadFile/<%=projectvo.getMovieurl()%>&LogoText=www.lanrentuku.com&BufferTime=3&IsAutoPlay=0',
									'quality',
									'high',
									'pluginspage',
									'http://www.macromedia.com/go/getflashplayer',
									'type', 'application/x-shockwave-flash',
									'movie', 'flvplayer.swf'); //end AC code
						</script>
					</div>
					<%} %>
					<h2>
						<p style="font-weight:normal;">
							<span style="font-size:18px; line-height: 2;"><%=projectvo.getDetaildescription()%></span>
							
						</p>
						<br>
						<%
							String url = projectvo.getDescriptionimages();
						...
						<p style="font-weight:normal;">
							<img alt="test" border="0" src="uploadFile/<%=urls[i]%>"
								style="display: block;">
						</p>
						<%
							}
					...
					<div class="z-corner">
						<span>分类：<%=ProjectCatagoryMgr.getInstance()
					.getProjectCatagoryVOById(projectvo.getCatagoryid())
					.getCatagoryname()%> </span>
					</div>
				</div>
			</div>
			<div class="m-portpe">
